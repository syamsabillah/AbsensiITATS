...
import AddUserModal from '../components/modal/tambahakun';
import EditUserModal from '../components/modal/editakun';
...
const ECommerce: React.FC = () => {
  ...
  const [users, setUsers] = useState([
    { nama: '', no_induk: '', penjurusan: '', no_telp: '', role: '', id: '' },
  ]);
  ...
  const [selectedUserId, setSelectedUserId] = useState(''); // State to store selected user for deletion
  const [selectedUserRole, setSelectedUserRole] = useState(''); // State to store selected user for deletion
  const handleDeleteUser = async () => {
    try {
      await axios.delete(`http://localhost:5000/user/${selectedUserId}`);
      ...
      <div className="flex flex-col">
        <div className="flex flex-row pl-6 pt-4 pb-6 bg-white dark:border-strokedark dark:bg-boxdark space-y-2 w-full">
          ...
          <div className="flex w-full justify-end">
            <div
              className="flex space-x-3 active:scale-[.95] w-auto my-auto h-11 pl-6 pr-6 mr-4 dark:border-white border border-placeholder rounded-md text-black text-md cursor-pointer"
              onClick={() => setIsModalOpen(true)}
            >
              <div className="font-medium my-auto dark:text-white">
                Tambah Akun
              </div>
            </div>
          </div>
